...
ARG VCS_REF=697f085
ARG GRAYLOG_VERSION=4.0.0-SNAPSHOT
...
COPY graylog-${GRAYLOG_VERSION}.tgz /tmp/graylog-${GRAYLOG_VERSION}.tgz

RUN \
  mkdir /opt/graylog && \
  tar --extract --gzip --file "/tmp/graylog-${GRAYLOG_VERSION}.tgz" --directory /opt/graylog
